        ...
        <table class="table table-bordered table-hover textCenter" id="records_table">
            <thead >
                <tr>
                    ...
                    <th class="textCenter">{{ __('dashboard.Quntity') }}</th>
                </tr>
            </thead>
            <tbody>
                @foreach($info as $product)
                <?php 
                    $pro =\App\Models\Product::whereId($product->product_id)->first(); 
                    $old_price = $pro->price;
                    $new_price = $pro->price;
                    if($pro->discount){
                ...
                <tr>
                    ...
                    <td class="textCenter">{{ $product->qty }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
